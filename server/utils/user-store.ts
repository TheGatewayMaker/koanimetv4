...
export async function createUser(
  user: Omit<UserRecord, "createdAt" | "watchHistory">,
): Promise<UserRecord> {
  const now = Date.now();
...
export async function upsertWatchProgress(
  ...
  if (USE_SUPABASE) {
    try {
      ...
      const rows = await supabaseSelect(
        "watch_history",
        `?user_id=eq.${encodeURIComponent(userId)}&select=anime_id,episode,position,title,image,updated_at`,
      );
      return (
        Array.isArray(rows)
          ? rows.map((r: any) => ({
...
export async function getWatchHistory(userId: string): Promise<WatchEntry[]> {
  if (USE_SUPABASE) {
    try {
      const rows = await supabaseSelect(
        "watch_history",
        `?user_id=eq.${encodeURIComponent(userId)}&select=anime_id,episode,position,title,image,updated_at&order=updated_at.desc`,
      );
  ...
  return (
    u?.watchHistory?.slice().sort((a, b) => b.updatedAt - a.updatedAt) || []
  );
}
